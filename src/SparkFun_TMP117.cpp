...
*/
bool TMP117::begin(TwoWire &wirePort, uint16_t address)
{
    ...
    // DEVICE_ID should always be 0x117
	// Checks to see if properly connected
    if (deviceIDValue != DEVICE_ID_VALUE)
    {
        return false;
    }
...
{
	_i2cPort->beginTransmission((uint16_t)_deviceAddress);
...
uint16_t TMP117::readRegister(TMP117_Register reg)
{
    ...
    _i2cPort->requestFrom(_deviceAddress, (byte)2); // Ask for 2 byte, once done, bus is released by default
...
*/
uint16_t TMP117::writeRegisters(TMP117_Register reg, byte *buffer, byte len)
{
    _i2cPort->beginTransmission(_deviceAddress);
    _i2cPort->write(reg);
    for (int i = 0; i < len; i++)
...
void TMP117::conversionMode()
{
	...
	writeRegisters(cycle, 00, 2); // Continuous Conversion (CC)
...
void conversionCycleTime()
{
	...
	writeRegisters(cycle, 000, 3); // Sets the conversion cycle time to be between 15.5ms and 1s
